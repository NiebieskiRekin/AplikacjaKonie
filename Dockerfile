...
RUN --mount=type=cache,target=/root/.npm,id=npm-all-deps npm pkg delete scripts.prepare && npm ci
COPY . .
RUN npm run build
...
COPY --link package.json package-lock.json /app/
COPY --link apps/backend/package.json /app/apps/backend/package.json
RUN --mount=type=cache,target=/root/.npm,id=npm-prod-deps npm pkg delete scripts.prepare && npm ci --omit=dev --workspace apps/backend
