...
function AddEvent() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response1 = await APIClient.konie.$get();
        ...
        if (type === "podkucie") {
          const peopleRes = await APIClient.kowale.$get();
          if (peopleRes.ok) {
          ...
          const peopleRes = await APIClient.weterynarze.$get();
          if (peopleRes.ok) {
            const peopleData = await peopleRes.json();
            setPeople(peopleData);
          } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (type === "podkucia") {
        ...
        const response = await APIClient.wydarzenia.podkucie.$post({
          json: payload,
        });
