...
import { users, usersInsertSchema, userPermissionsInsertSchema, user_permissions } from "../db/schema";
...
}).post("/permission", zValidator("json", userPermissionsInsertSchema), 
  async (c) => {
    try {
      var user_permission = c.req.valid("json");
      const [added_permission] = await db.insert(user_permissions).values(user_permission).returning();
      if (!added_permission) {
        return c.json({ error: "Błąd dodawania użytkownika." }, 401);
      }
